...
class Jawabanlatihan extends BaseController
{
    ...
    public function index()
    {
        if ($this->session->get("user_nm") == "") {
            ...
            return view('admin/jawabanlatihan',$data);
        }
    ...
    public function showjawaban() {
        ...
        $this->session->set("jenis_filter",$jenis);

        $resjawaban = $this->latihanmodel->getJawabanbyjenis($jenis)->getResult();
          ...
          $no = 1;
              foreach ($resjawaban as $key) {
                if ($key->jawaban_img == "") {
                  ...
                  $jawaban_img = "<img style='max-width:200px;heigth:100%;' src='".base_url()."/images/jawaban/jenis/".$key->jenis_id."/".$key->jawaban_img.".jpg'>";
                }
                $ret .= "<tr>
                        ...
                        <td style='text-align:center;'><button onclick='editjawaban(".$key->jawaban_id.",".$key->jenis_id.")' style='font-size:10px;' class='btn btn-secondary' data-toggle='modal' data-target='#modal-lg'>Edit</button> <button onclick='hapusjawaban(".$key->jawaban_id.")' style='font-size:10px;' class='btn btn-danger'>Hapus</button></td>
                        </tr>";
    ...
    public function tambahjawaban() {
        $ret = "<div class='card'>
                ...
                <div class='form-group'>
                    ...
                    $ressoal = $this->latihanmodel->getSoalAll()->getResult();
                    foreach ($ressoal as $sl) {
                        $optsoal .= "<option data-jenis='$sl->jenis_id' value='".$sl->soal_id."'>".$sl->no_soal."). [".$sl->jenis_nm."] ".$sl->soal_nm."</option>";
                    }
    ...
    public function editjawaban() {
        ...
        $jenis = $this->request->getPost('jenis');
        $resjawaban = $this->latihanmodel->getJawabanbyid($jawaban_id)->getResult();
        foreach ($resjawaban as $key) {
            $ret = "<div class='card'>
                <div class='card-body'>
                <div class='row'>
                ...
                <div class='form-group'>
                    ...
                    $ressoal = $this->latihanmodel->getSoalAll()->getResult();
                    foreach ($ressoal as $sl) {
                        $optsoal .= "<option ".($sl->soal_id==$key->soal_id?'selected':'')." data-jenis='$sl->jenis_id' value='".$sl->soal_id."'>".$sl->no_soal."). [".$sl->jenis_nm."] ".$sl->soal_nm."</option>";
                    }
                    $ret .= "<div class='form-group row'>
                        <label>Soal</label>
                        <select id='soal_id' class='form-control select2' style='width: 100%;'>
                    ...
                    <div class='form-group row'>
                        ...
                        if ($key->jawaban_img == "") {
                            ...
                            $ret .= "<img src='".base_url()."/images/jawaban_latihan/jenis/$jenis/".$key->jawaban_img."' style='width: 200px;height: 100%;'>";
                        }
                $ret .= "</div>
                    </div>
                    <div class='card-footer'>
    ...
    public function simpanjawaban() {
        ...
        $simpan = $this->latihanmodel->simpanjawaban($data);
        if ($simpan) {
            return "sukses";
        } else {
            return "gagal";
    ...
    public function updatejawaban() {
        ...
        $res = $this->latihanmodel->updatejawaban($jawaban_id,$data);
        if ($res) {
            echo "sukses";
        } else {
            echo "gagal";
    ...
    public function hapusjawaban() {
        $jawaban_id = $this->request->getPost('jawaban_id');
        $res = $this->latihanmodel->hapusjawaban($jawaban_id);
        if ($res) {
            echo "sukses";
        } else {
            echo "gagal";
